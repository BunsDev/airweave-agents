...
class SearchServiceV2:
    ...
    async def _persist_search_data(
        ...
        try:
            ...
            # Track search persistence in PostHog
            from airweave.analytics.search_analytics import track_search_persisted

            # Extract search configuration flags
            query_expansion_enabled = (
                getattr(search_request, "expansion_strategy", None) != "no_expansion"
            )
            reranking_enabled = getattr(search_request, "enable_reranking", None) is not False
            query_interpretation_enabled = (
                getattr(search_request, "enable_query_interpretation", None) is not False
            )

            track_search_persisted(
                ctx=ctx,
                ...
                results_count=len(search_response.results),
                status=status,
                search_type="regular",  # Could be enhanced to detect streaming
                response_type=(
                    search_request.response_type.value if search_request.response_type else None
                ),
